...
import type { Space } from '@app/tokens'
...
const Container = styled.div<{ $size: Size }>(
  ({ theme, $size }) => css`
    ...
    label {
      ...
      font-weight: ${$size === 'extraSmall' ? theme.fontWeights.normal : theme.fontWeights.bold};
      display: flex;
      align-items: center;
    ...
    label#eth {
      ...
      transform: translate(-50%, -50%) translateX(-${theme.space[KNOB_SIZES[$size].translateX]});
    }
...
const InputComponent = styled.input<{ $size?: Size }>(
  ({ theme, $size = 'medium' }) => css`
    ...
    border-radius: ${$size === 'extraSmall' ? theme.radii.full : theme.radii.large};
    ...
    &::after {
      ...
      width: ${theme.space[KNOB_SIZES[$size].width]};
      height: ${theme.space[KNOB_SIZES[$size].height]};
      border-radius: ${$size === 'extraSmall' ? theme.radii.full : theme.space['1.5']};
