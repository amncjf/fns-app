...
describe('onwershipInfoCalc', () => {
  ...
  it('if unwrapped and expired, should return manager and expiry', () => {
    ...
    const result = ownershipInfoCalc('fil', false, owners, gracePeriodEndDate, expiryDate, 'en-US')
  ...
  it('If not expired, show owners and expiry', () => {
    ...
    const result = ownershipInfoCalc(
      'test.fil',
      false,
      owners,
      gracePeriodEndDate,
    ...
    const expectedExpiryLabel = expiryDate.toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    })
  ...
  it('should return parent as [root] if TLD', () => {
    const result = ownershipInfoCalc('fil', true, [], new Date(), new Date())
    expect(result).toEqual([
      {
        key: 'name.owner',
        type: 'text',
