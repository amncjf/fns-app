...
mockUsePrimary.mockImplementation((address, skip) => {
  ...
  return {
    ...
    name: skip ? undefined : isNoPrimary ? null : 'primary.fil',
    beautifiedName: isNoPrimary ? null : 'primary.fil',
    loading: false,
  }
})
...
describe('<OwnerProfileButton/>', () => {
  it('renders', () => {
    render(<OwnerProfileButton iconKey="name.owner" value="name.fil" />)
  ...
  describe('address', () => {
    ...
    it('should display primary if address does have primary name', async () => {
      ...
      const element = screen.getByTestId('owner-profile-button-name.owner')
      expect(element).toBeInTheDocument()
      expect(element).toHaveTextContent(`name.ownerprimary.fil`)
    })
  })
  ...
  describe('parent', () => {
    ...
    it('should display link to /tld/eth if value is tld', () => {
      render(<OwnerProfileButton iconKey="name.parent" value="fil" />)
      const element = screen.getByTestId('owner-profile-button-name.parent')
      expect(element).toBeInTheDocument()
      expect(element).toHaveTextContent(`name.parentfil`)
      expect(element).toHaveAttribute('href', '/tld/fil')
    })

    it('should display link to /name.fil if value is not tld', () => {
      render(<OwnerProfileButton iconKey="name.parent" value="name.fil" />)
      const element = screen.getByTestId('owner-profile-button-name.parent')
      expect(element).toBeInTheDocument()
      expect(element).toHaveTextContent(`name.parentname.fil`)
      expect(element).toHaveAttribute('href', '/name.fil')
    })
  })
})
